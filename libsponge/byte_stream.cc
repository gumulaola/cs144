...
ByteStream::ByteStream(const size_t capacity)
    : _queue(), _total_write(0), _total_read(0), _cap(capacity), _cur_size(0), _end_input(false), _error(false) {}
...
size_t ByteStream::write(const string &data) {
    ...
    size_t write_size = min(len, remain);
...
size_t ByteStream::remaining_capacity() const { return _cap - _queue.size(); }
